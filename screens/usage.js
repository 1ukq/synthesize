...
export default class Pause extends React.Component{
  ...
  componentDidMount = async () => {
    ...
    this.navigation.state.params = {mute: false}
    ...
    await this.voiceOver.loadAsync(require('../assets/sound/voiceOver/usage.mp3'), status, false);
  ...
  _goTo = async (routeName) => {
    ...
    if(routeName == NAME) {
      ...
      this.voiceOver = new Audio.Sound();
      await this.voiceOver.loadAsync(require('../assets/sound/voiceOver/usage.mp3'), {
        shouldPlay: false,
        androidImplementation:'MediaPlayer'
      }, false);
    }
